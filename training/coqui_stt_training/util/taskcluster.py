...
def maybe_download_tc(target_dir, tc_url, progress=True):
    ...
    if not os.path.isfile(target_file):
        ...
        _, headers = urllib.request.urlretrieve(
            tc_url, target_file, reporthook=(report_progress if progress else None)
        )
...
def maybe_download_tc_bin(**kwargs):
    final_file = maybe_download_tc(
        kwargs["target_dir"], kwargs["tc_url"], kwargs["progress"]
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Tooling to ease downloading of components from TaskCluster."
    )
    parser.add_argument(
        "--target", required=False, help="Where to put the native client binary files"
    )
    parser.add_argument(
        "--arch",
        required=False,
    ...
    maybe_download_tc(
        target_dir=args.target, tc_url=get_tc_url(args.arch, args.artifact, args.branch)
    )
